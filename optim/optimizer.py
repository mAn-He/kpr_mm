...
def build_optimizer(
    ...
    if staged_lr and transreid_lr:
        base_lr = lr
        param_groups = []
        for key, value in model_for_params.named_parameters():
            if not value.requires_grad:
                continue
            if key.startswith("module.backbone_appearance_feature_extractor.model"):  # TODO generic - only work with promptable_trans
                _lr = reduced_lr
                _wd = weight_decay
                if "bias" in key:
                    _lr = reduced_lr * base_lr_mult
                    _wd = weight_decay_bias
            else:
                _lr = base_lr
                _wd = weight_decay

            param_groups += [{"params": [value], "lr": _lr, "weight_decay": _wd}]
    elif staged_lr:
        ...
        for name, module in model_for_params.named_children():
    ...
    elif transreid_lr:  # This one is used by KPR
        base_lr = lr
        param_groups = []
        for key, value in model_for_params.named_parameters():
            if not value.requires_grad:
            ...
            if "bias" in key:  # TODO do it only for backbone feature extractor
                lr = base_lr * base_lr_mult

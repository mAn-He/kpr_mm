...
#             if target_masks.shape[2:] != pixels_cls_scores.shape[2:]:
...
#         # 디버깅: 해상도 정보 출력
#         print(f"compute_pixels_cls_accuracy - pixels_cls_scores shape: {pixels_cls_scores.shape}")
#         print(f"compute_pixels_cls_accuracy - target_masks shape: {target_masks.shape}")
        
#         # 중요: target_masks를 pixels_cls_scores의 크기에 맞게 리사이징
#         if target_masks.shape[2:] != pixels_cls_scores.shape[2:]:
#             print(f"Resizing target_masks from {target_masks.shape[2:]} to {pixels_cls_scores.shape[2:]}")
#             target_masks = nn.functional.interpolate(
#                 target_masks,
#                 pixels_cls_scores.shape[2:],
#                 mode="bilinear",
#                 align_corners=True,
#             )
#             print(f"After resize, target_masks shape: {target_masks.shape}")
        
#         pixels_cls_score_targets = target_masks.argmax(dim=1)  # [N, Hf, Wf]
        
#         # 디버깅: 변환 후 형태 확인
#         print(f"pixels_cls_score_targets shape: {pixels_cls_score_targets.shape}")
#         print(f"pixels_cls_scores shape after permute: {pixels_cls_scores.permute(0, 2, 3, 1).flatten(0, 2).shape}")
...
#         # 크기 재확인 및 안전장치
#         if pixels_cls_score_targets.shape[0] != pixels_cls_scores.shape[0]:
#             print(f"Warning: Size mismatch after flattening!")
#             print(f"  targets: {pixels_cls_score_targets.shape}")
#             print(f"  scores: {pixels_cls_scores.shape}")
            
#             # 더 작은 크기에 맞춰서 자르기
#             min_size = min(pixels_cls_score_targets.shape[0], pixels_cls_scores.shape[0])
#             pixels_cls_score_targets = pixels_cls_score_targets[:min_size]
#             pixels_cls_scores = pixels_cls_scores[:min_size]

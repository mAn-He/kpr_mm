...
from torchreid.metrics.minp import calculate_minp # Added import
...
class Engine(object): # Consider renaming class if needed, e.g., EngineWithMINP
    """A generic base Engine class for both image- and video-reid.
       Includes mINP calculation.
    ...
    def __init__(self, config, datamanager, writer, engine_state, use_gpu=True, save_model_flag=False, detailed_ranking=False):
        ...
        self.best_mAP = 0.0 # Add variable to track best mAP
    ...
    # Note: save_model might need adjustment if mINP should be saved in checkpoint
                ...
                save_checkpoint(
                    {
                        ...
                        'epoch': epoch + 1,
                        'rank1': cmc, # Assuming cmc is rank1 or cmc[0]
                        'mAP': mAP,
                        # 'mINP': mINP, # Add mINP here if needed
                        'ssmd': ssmd,
                        'config': self.config,
    ...
    def run(
        ...
        for epoch in range(self.engine_state.start_epoch, self.engine_state.max_epoch):
            ...
            if self.writer.intermediate_evaluate():
                ...
                # Added mINP to return values
                rank_1, mAP, mINP, ssmd = self.test(
                    epoch,
                    dist_metric=dist_metric,
                    normalize_feature=normalize_feature,
                    visrank=False,
                ...
                if is_best:
                    self.best_mAP = mAP
                    print(f"*** New best mAP: {self.best_mAP:.2%}. Saving best model ***")

                # Pass is_best flag to save_model
                self.save_model(epoch, rank_1, mAP, ssmd, save_dir, is_best=is_best)
        ...
        if self.engine_state.max_epoch > 0:
            ...
            # Pass is_best flag to save_model
            self.save_model(self.engine_state.epoch, rank_1, mAP, ssmd, save_dir, is_best=is_best)

        self.writer.total_run_timer.stop()
        self.engine_state.run_completed()
        self.logger.close()

        return mAP # Or return mAP, mINP?
    ...
    def test(
        ...
        mINP_avg = AverageMeter() # Added mINP AverageMeter
        ...
        cmc_per_dataset = {}
        mAP_per_dataset = {}
        mINP_per_dataset = {} # Added mINP per dataset dict
        ssmd_per_dataset = {}
        pxl_acc_per_dataset = {}
        for name in targets:
            ...
            if is_source_dataset or not evalate_on_sources_only:
                ...
                # Added mINP to the return values
                cmc, mAP, mINP, ssmd, avg_pxl_pred_accuracy = self._evaluate(
                    epoch,
                    dataset_name=name,
                ...
                # Added mINP to report_performance call (assuming writer handles it or ignores extra args)
                self.writer.report_performance(cmc, mAP, mINP, ssmd, avg_pxl_pred_accuracy, dataset_nickname)

                cmc_per_dataset[dataset_nickname] = perc(cmc)
                mAP_per_dataset[dataset_nickname] = perc(mAP)
                mINP_per_dataset[dataset_nickname] = perc(mINP) # Added mINP
                ...
                if is_source_dataset:
                    cmc_avg.update(cmc)
                    mAP_avg.update(mAP)
                    mINP_avg.update(mINP) # Added mINP update
        ...
        mINP_per_dataset[average_score_key] = np.array(list(mINP_per_dataset.values())).mean() # Added mINP avg
        ...
        if mAP_avg.count != 0:
            # Added mINP_avg.avg to report_performance call
            self.writer.report_performance(cmc_avg.avg, mAP_avg.avg, mINP_avg.avg, ssmd_avg.avg, pxl_acc_avg.avg)
        # Added mINP_per_dataset to report_global_performance call
        ...
        # Added mINP_avg.avg to return statement
        return r1, mAP_avg.avg, mINP_avg.avg, ssmd_avg.avg
    ...
    def _evaluate(
        ...
        # Calculate mINP
        print('Computing mINP ...')
        mINP = calculate_minp(distmat, q_pids, g_pids, q_camids, g_camids, max_rank=ranks[-1]) # Use max rank from ranks list

        mAP = eval_metrics['mAP']
        cmc = eval_metrics['cmc']
        print('** Results **')
        print('mAP: {:.2%}'.format(mAP))
        print('mINP: {:.2%}'.format(mINP)) # Print mINP
        print('CMC curve')
        for r in ranks:
        ...
        # Added mINP to return statement
        return cmc, mAP, mINP, ssmd, 0
    ...
    def two_stepped_transfer_learning(
        ...
        if isinstance(model, KPR) and isinstance(model.backbone_appearance_feature_extractor, PromptableTransformerBackbone):
